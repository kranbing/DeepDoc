...
def structure_comment_for_path(path: Path) -> str:
    ...
    if name == "chunks.json":
        return "单文档标准化块文件，按页保存 OCR 块及其规范化文本，供检索、邻近上下文和向量索引使用。"
    if name == "quality_report.json":
        return "单文档OCR黑盒质量报告文件，记录文档级与页级评分、问题项、版面指标和重试信息。"
    if name == "manifest.json" and "/documents/_vector_index/" in path_str:
        return "项目级向量索引清单，记录向量模型、索引覆盖文档与块元信息，用于基础语义检索。"
    return ""
