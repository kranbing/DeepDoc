...
@app.get("/api/kg/graph.json")
def get_knowledge_graph_json() -> Dict[str, Any]:
    """Serve the knowledge graph data."""
    kg_path = ROOT / "test" / "lad_test" / "kg_graph.json"
    if not kg_path.is_file():
        return {"nodes": [], "edges": [], "metadata": {"error": "Knowledge graph not generated. Run: python test/run_kg_pipeline.py"}}
    import json as _json
    with open(kg_path, "r", encoding="utf-8") as f:
        return _json.load(f)


@app.get("/api/kg/graph.html")
def get_knowledge_graph_page() -> FileResponse:
    """Serve the knowledge graph visualization page."""
    html_path = ROOT / "test" / "kg_design" / "kg_graph_frontend.html"
    if not html_path.is_file():
        raise HTTPException(status_code=404, detail="Knowledge graph visualization not found")
    return FileResponse(html_path, media_type="text/html; charset=utf-8")
