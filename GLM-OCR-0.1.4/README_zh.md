...
> **Windows + Docker GPU 完整步骤（构建镜像、拉模型、启动 vLLM、Python 测试、版面分块与故障排查）** 见：  
> **[docs/自托管Docker与测试完整指南_zh.md](docs/自托管Docker与测试完整指南_zh.md)**  
> **网页前端（React）+ FastAPI 与 vLLM + `glmocr.server` 的链式部署** 见该文档 **§11** 与 **`apps/start-web-stack.ps1`**。
...
显存紧张时，可显式降低上下文与占用（`max_model_len` 默认可能很大，KV 会占大量显存）：

```bash
vllm serve zai-org/GLM-OCR --allowed-local-media-path / --port 8080 --speculative-config '{"method": "mtp", "num_speculative_tokens": 1}' --served-model-name glm-ocr \
  --max-model-len 8192 --gpu-memory-utilization 0.75 --max-num-seqs 4
```

仍 OOM 时可将 `--max-model-len` 再降到 `4096` / `2048`，或略调低 `--gpu-memory-utilization`。Windows Docker 可参考仓库内 `docker-glmocr-gpu/run-vllm-daemon.ps1` 中的变量。
