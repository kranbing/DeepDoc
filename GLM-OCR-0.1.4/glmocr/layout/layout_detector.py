...
def resolve_layout_model_dir(model_dir: Optional[str]) -> str:
    """Resolve HF repo id or local path to a directory that contains model weights.

    The Hub repo only ships ``model.safetensors`` (no ``pytorch_model.bin``). A
    partial cache (only config/preprocessor) often triggers misleading
    ``pytorch_model.bin`` errors; for repo ids we call ``snapshot_download`` to
    ensure the full snapshot (including weights) is present.
    """
    if not model_dir or not str(model_dir).strip():
        raise ValueError("layout.model_dir is not set")
    raw = str(model_dir).strip()
    p = Path(raw)
    if p.exists() and p.is_dir():
        st = p / "model.safetensors"
        binf = p / "pytorch_model.bin"
        if st.is_file() or binf.is_file():
            return str(p.resolve())
        raise FileNotFoundError(
            f"本地版面模型目录缺少权重文件: {p}\n"
            "请放入 model.safetensors，或执行:\n"
            "  huggingface-cli download PaddlePaddle/PP-DocLayoutV3_safetensors "
            "--local-dir D:\\models\\PP-DocLayoutV3_safetensors\n"
            "然后设置 GLMOCR_LAYOUT_MODEL_DIR 或 config 里 model_dir 为该目录。"
        )
    try:
        from huggingface_hub import snapshot_download
    except ImportError as e:
        raise ImportError(
            "需要 huggingface_hub 以下载版面模型。请执行: pip install huggingface_hub"
        ) from e

    # Windows 上默认 HF 缓存常因 symlink 降级导致大文件不完整；固定到本地目录并禁用 symlink，更稳。
    override = os.environ.get("GLMOCR_LAYOUT_SNAPSHOT_DIR", "").strip()
    if override:
        target = Path(override).expanduser().resolve()
    else:
        safe = raw.replace("/", "__")
        target = (Path.home() / ".cache" / "glmocr" / safe).resolve()
    target.mkdir(parents=True, exist_ok=True)

    logger.info(
        "正在拉取版面模型完整快照 %s → %s（若中断可重试，支持断点续传）…",
        raw,
        target,
    )
    try:
        ...
        raise RuntimeError(
            f"无法下载版面模型 {raw!r}。底层错误: {e!r}\n\n"
            "可依次尝试：\n"
            "  1) pip install -U hf_xet huggingface_hub   （大文件常用 xet 加速/分块）\n"
            "  2) 设置 HF_ENDPOINT=https://hf-mirror.com 后重试\n"
            "  3) 手动下载到目录后设 GLMOCR_LAYOUT_MODEL_DIR：\n"
            "     huggingface-cli download PaddlePaddle/PP-DocLayoutV3_safetensors "
            "--local-dir D:\\\\models\\\\PP-DocLayoutV3_safetensors\n"
            f"  4) 或设置 GLMOCR_LAYOUT_SNAPSHOT_DIR 为自定义下载目录（当前默认: {target}）"
        ) from e
    return str(target)
...
class PPDocLayoutDetector(BaseLayoutDetector):
    ...
    def start(self):
        ...
        resolved_dir = resolve_layout_model_dir(self.model_dir)
        # HF 仓库仅有 model.safetensors（无 pytorch_model.bin）
