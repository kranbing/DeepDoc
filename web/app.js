...
function updateDocumentStatusBarVisibility(doc = getSelectedDoc()) {
  ...
  if (!top) return;
  const hideForGraph = !!(state.viewMode === "document" && doc?.isPdf && (doc.pdfViewMode === "graph" || doc.pdfViewMode === "kg"));
...
function renderDocHeader() {
  ...
  if (doc.isPdf && doc.ocrParsed) {
    const viewLabel = doc.pdfViewMode === "kg" ? "知识图谱 · " : doc.pdfViewMode === "graph" ? "图谱视图 · " : doc.pdfViewMode === "parsed" ? "解析视图 · " : "原文视图 · ";
    $("#docSubmeta").textContent =
      viewLabel +
...
function renderDocumentContinuous() {
  ...
  if (doc && doc.isPdf) {
    ...
    if (doc.pdfViewMode === "kg") {
      pagesWrap.innerHTML = "";
      delete pagesWrap.dataset.pdfDocId;
      delete pagesWrap.dataset.pdfViewKey;
      const frame = document.createElement("iframe");
      frame.className = "lad-graph-frame";
      frame.title = "Knowledge Graph";
      frame.src = `${API_BASE}/api/kg/graph.html`;
      pagesWrap.appendChild(frame);
      $("#pageTotal").textContent = "/ 1";
      $("#pageNum").value = "1";
      renderViewSwitcher(doc);
...
function setupEventHandlers() {
  ...
  // KG button: always available (independent of document OCR state)
  $("#stageKg")?.addEventListener("click", () => {
    const doc = getSelectedDoc();
    if (doc?.isPdf) {
      setDocumentPdfViewMode("kg");
    }
  });
