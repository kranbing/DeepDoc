...
def main() -> int:
    ...
    if args.mode == "mock":
        adapter = MockOcrAdapter(noisy=args.mock_noisy)
    else:
        ...
        if args.enable_mock_fallback:
            adapter = FailoverOCRAdapter(adapter, MockOcrAdapter(noisy=args.mock_noisy))
    ...
    try:
        ...
        result = pipeline.process_document(args.input, doc_id=args.doc_id)
