...
sys.path.insert(0, str(PERSON_DIR / "src"))

from entity_extractor import extract_entities_from_document
from relation_extractor import extract_relations_from_chunk, extract_structural_relations
from cleaner import clean_entities, clean_relations, generate_cleaning_report
...
def main():
    ...
    print("\n--- Entity Extraction ---")
    t0 = time.time()
    raw_entities = extract_entities_from_document(chunks, use_llm=False, max_llm_calls=0, llm_batch_size=3)
    t1 = time.time()
